      ...
      const func = async () => {
        try {
          ...
          toast({
            ...
            description:
              "Please try again in sometime or try after refreshing website",
            variant: "destructive",
          });
          resolve("");
        }
...
function Certifications() {
  ...
  const { mutate: deleteCertificate } = useMutation({
    mutationFn: async (_id) => {
      await axiosPrivate.delete(`/deleteCertificates/${user._id}/${_id}`);
      await queryClient.refetchQueries({ queryKey: ["user"] });
    },
  });
    ...
    >
      <Paper>
        ...
        <div className="space-y-4">
          {profile.certificate && user.certificate?.length !== 0 ? (
