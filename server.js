...
app.get('/bonuses', authenticate, async (req, res) => {
  try {
    ...
    const availableBonuses = bonuses.filter(
      bonus => !user.bonuses.includes(bonus._id),
    );
...
app.post(
  ...
  [
    ...
    body('education.*.startYear').isInt({
      min: 1900,
      max: new Date().getFullYear(),
    }),
    body('education.*.endMonth').trim().escape(),
    body('education.*.endYear').isInt({
      min: 1900,
    ...
    body('experience.*.startYear').isInt({
      min: 1900,
      max: new Date().getFullYear(),
    }),
    body('experience.*.endDate').trim().escape(),
    body('experience.*.endYear').isInt({
      min: 1900,
      max: new Date().getFullYear(),
    }),
